...
(function (factory) {
    ...
    $.widget('blueimp.fileupload', {
        ...
        _initXHRData: function (options) {
            ...
            var date = new Date();
            var unique = date.getUTCFullYear() + "-" + date.getUTCMonth() + "-" + date.getUTCDate() + "-" + Math.uuid(8);
            filenameParts.splice(-1, 0, unique);
            if (filenameParts.length > 1) {
                var ext = filenameParts.splice(-1, 1);
                filename = filenameParts.join(".") + "-" + unique + "." + ext
            }
            else {
            ...
            if (!multipart) {
                ...
                if (options.postMessage) {
                    ...
                    if (options.blob) {
                        options.headers['Content-Disposition'] = 'attachment; filename="' +
                            encodeURI(filename) + '"';
                        formData.append(paramName, options.blob, filename);
                    } else {
                        $.each(options.files, function (index, file) {
